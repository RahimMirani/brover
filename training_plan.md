...
**Phase 3 — Place Teaching [DONE].** Domain layer (`backend/teaching.py`, `backend/localization.py`) plus seven voice-driven tools: `remember_here` (stationary stop-and-say) and `start_tour` / `tag_place` / `end_tour` (continuous capture while the user drives), with `find_place`, `localize` (confidence-classified — empty / confident / ambiguous / unrecognized), and `forget_place` on the read side. Shared SQLite connection owned by `main.py`'s lifespan; introspection via `GET /api/places`. Shipped as PR #6. Localize thresholds (HIGH=0.25, LOW=0.45, margin=0.05) are conservative starting points and want recalibration once a real apartment has been taught.

**Phase 4 — Route Recording [DONE].** `backend/route_recording.py` records (frame, motor-action) sequences while the user drives manually; mode hooks forward every teleop command into the recorder and e-stop cancels cleanly. Four new tools: `start_route_recording`, `stop_route_recording`, `find_route` (read-only direct lookup), `list_routes`. Action format is encoded in one place (`encode_action` / `decode_action`) so Phase 5's replay parses exactly what was written. Bounded caps (`MAX_STEPS=50`, `MAX_SECONDS=120`) prevent a forgotten recording from burning Voyage quota. The existing `routes` + `route_steps` + `route_step_vectors` schema from Phase 2 was used as-is. `GET /api/routes` mirrors the places endpoint. Shipped as PR #7.

**Phase 5 — Graph Navigation [NEXT].** Build the place/route graph in memory, run shortest-path search for unseen pairs, and replay edges via teach-and-repeat with continuous visual re-localization between steps. New tool `execute_route` is the headline; `find_route` from Phase 4 grows to do graph composition when no direct route exists. Replay calls `decode_action` from Phase 4 so action parsing has one source of truth. Critical: `execute_route` must honor the existing `cancel_event` between every step so manual override and e-stop keep working — long-running navigation must not become a silent override.
